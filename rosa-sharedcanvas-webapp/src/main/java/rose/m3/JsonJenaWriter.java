...
/*
 * Hacked from code at: http://n2.talis.com/svn/jena/rdf-writers/trunk/src/main/java/com/talis/rdfwriters/json/JSONJenaWriter.java
 *  
...
public class JsonJenaWriter implements RDFWriter {
	...
	public Object setProperty(String propName, Object propValue) {
		if (!(propValue instanceof String)) {
			// LOG.warn("JSON.setProperty: Property for '" + propName
			// + "' is not a string");
	...
	private void processSubject(Resource subjectResource) throws IOException {
		...
		if (subjectResource.isAnon()) {
			write("_:");
			write(escape(subjectResource.asNode().getBlankNodeId()
					.getLabelString()));
		} else {
	...
	private void processProperties(Resource subjectResource) throws IOException {
		...
		while (allPropertiesIterator.hasNext()) {
			...
			if (!propertiesToProcess
					.contains(statement.getPredicate().getURI())) {
		...
		for (Property property : propertiesToProcess) {
			...
			write("\" : [ ");
			StmtIterator propertyIterator = subjectResource
