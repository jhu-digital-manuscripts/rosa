...
public class BasePanelView extends Composite implements
    ...
    private final Label move_down_label;
    private final Label move_up_label;
    ...
    public BasePanelView() {
        ...
        move_up_label = new Label(Messages.INSTANCE.moveUp());
        move_up_label.addStyleName("Swap");

        move_down_label = new Label(Messages.INSTANCE.moveDown());
        move_down_label.addStyleName("Swap");
    ...
    private void setup_annotations_list() {
        ...
        anno_popup.addCloseHandler(new CloseHandler<PopupPanel>() {
    ...
    private void setup_meta_list() {
        ...
        meta_popup.addCloseHandler(new CloseHandler<PopupPanel>() {
            public void onClose(CloseEvent<PopupPanel> event) {
            ...
            public void onAttachOrDetach(AttachEvent event) {
                if (event.isAttached()) {
                    ...
                    tab_panel.setHeight((text_popup.getOffsetHeight()
                            - text_header.getOffsetHeight() - 16)
                            + "px");
                }
            }
        });

        text_popup.addCloseHandler(new CloseHandler<PopupPanel>() {
            public void onClose(CloseEvent<PopupPanel> event) {
    ...
    private void setup_options() {
        ...
        options_popup.addAttachHandler(new AttachEvent.Handler() {
            public void onAttachOrDetach(AttachEvent event) {
                if (event.isAttached()) {
                    ...
                    options_popup.setPopupPosition(
                            options_button.getAbsoluteLeft()
                                    + options_button.getOffsetWidth()
        ...
        options_popup.addCloseHandler(new CloseHandler<PopupPanel>() {
            public void onClose(CloseEvent<PopupPanel> event) {
    ...
    public void resize(int width, int height) {
        ...
        int popup_top = options_button.getAbsoluteTop()
                + options_button.getOffsetHeight();
        int right = options_button.getAbsoluteLeft()
                + options_button.getOffsetWidth();

        // set popup position
        text_popup.setPopupPosition((right - text_popup.getOffsetWidth()),
                popup_top);
        meta_popup.setPopupPosition((right - meta_popup.getOffsetWidth()),
                popup_top);
        anno_popup.setPopupPosition((right - anno_popup.getOffsetWidth()),
                popup_top);
        options_popup.setPopupPosition(
                (right - options_popup.getOffsetWidth()), popup_top);
    ...
    @Override
    public HasClickHandlers getMoveUpLabel() {
        return move_up_label;
    }

    @Override
    public HasClickHandlers getMoveDownLabel() {
        return move_down_label;
    }
}
